...
{
	int i,j,k;
	while(ch==' '||ch==10||ch==13) /* 忽略空格、换行和回车 */   //LinSixin 应该增加制表符 /t 判断 
	{
		getchdo;
	}
	if(ch>='a'&&ch<='z')
...
void init()
{
	...
	strcpy(&(word[3][0]),"do");
	strcpy(&(word[4][0]),"end");
	strcpy(&(word[5][0]),"for");			/*Linsixin 添加了新关键字这里要增加东西 而且要按照字符顺序插入 */ 
	strcpy(&(word[6][0]),"if");
	strcpy(&(word[7][0]),"odd");
	...
	wsym[3]=dosym;
	wsym[4]=endsym;
	wsym[5]=forsym;				/*Linsixin 添加了新关键字这里要增加东西 */ 
	wsym[6]=ifsym;
	wsym[7]=oddsym;
	wsym[8]=procsym;
	wsym[9]=readsym;
