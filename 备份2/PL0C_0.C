...
int statement(bool* fsys,int* ptx,int lev) /* 参数意义见block和enter函数 */
{
	...
	switch(sym)
	{
		...
		case forsym:	/*Linsixin 准备按照for语句处理 */ 
		{
			...
			getsymdo;
			if(sym!=lparen) error(19);	/*Linsixin 匹配左括号 错误号?  */ 
			getsymdo;
			if(sym!=varsym) error(61);
			else{
				getsymdo;
				vardeclarationdo(ptx,lev,&cx);					
				getsymdo;
				if(sym!=semicolon) error(20); 	/*Linsixin 匹配分号 错误号?  */ 
				else{
					memcpy(nxtlev,fsys,sizeof(bool)*symnum);
					nxtlev[semicolon]=true; 				/* 后跟符号为  ; */
					conditiondo(nxtlev,ptx,lev); 			/* 调用条件处理 */
					cx2=cx; /* 保存循环体的结束的下一个位置 */
					gendo(jpc,0,0); /* 生成条件跳转，但跳出循环的地址未知 */	
				}
				getsymdo;
	//			printf("sym = %s",sym);
				if(sym!=rparen) error(21); /*Linsixin 匹配右括号 错误号?  */ 
				else{
					getsymdo;	
